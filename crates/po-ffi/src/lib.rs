...
impl PoClient {
    pub fn new(
        bind_address_or_port: String,
        remote_address: Option<String>,
    ) -> Result<Self, PoFfiError> {
        ...
        let inner = rt
            .block_on(async {
                if let Some(remote) = remote_address {
                    ...
                    // Server mode
                    let port: u16 = bind_address_or_port
                        .parse()
                        .map_err(|_| po_node::node::PoError::Config("Invalid port".to_string()))?;
                    Po::bind(port).await
                }
            })
            .map_err(|e| match e {
    ...
    pub fn node_id(&self) -> String {
        self.rt
            .block_on(async { self.inner.lock().await.node_id() })
    }

    pub fn send(&self, data: Vec<u8>) -> Result<(), PoFfiError> {
        self.rt
            .block_on(async { self.inner.lock().await.send(&data).await })
            .map_err(|e| match e {
                po_node::node::PoError::Session(msg) => PoFfiError::Session(msg),
    ...
    pub fn close(&self) -> Result<(), PoFfiError> {
        self.rt
            .block_on(async { self.inner.lock().await.close().await })
            .map_err(|e| match e {
                po_node::node::PoError::Session(msg) => PoFfiError::Session(msg),
                _ => PoFfiError::Generic(e.to_string()),
            })
